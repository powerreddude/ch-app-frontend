          ...
          <div key={server.id} className="flex justify-center py-4">
            <Link to={`/s/${server.id}`}>
              <div className="w-36 h-36 p-3 drop-shadow-xl rounded-xl bg-zinc-700">
                ...
                <abbr title={server.name} className="relative bottom-4 text-zinc-50 px-1 shadow-xl bg-zinc-800 rounded-md overflow-hidden text-ellipsis max-w-36 border-b-0 no-underline block">
                  {server.name}
                </abbr>
              </div>
            </Link>
